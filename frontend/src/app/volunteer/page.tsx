...
export default function VolunteerDashboard() {
  ...
  // 1. Fetch both open tasks and tasks assigned to the current volunteer
  ...
  // 2. Volunteer accepts an unassigned task
  const handleAcceptTask = async (taskId: string) => {
    setError('');
    setSuccess('');

    try {
      await api.post(`/logistics/${taskId}/accept`);
  ...
  // 3. Verify Handover OTP to complete delivery
  const handleVerifyOtp = async () => {
    ...
    try {
      await api.post(`/logistics/${selectedTaskForOtp._id}/complete-otp`, {
        otp: otpInput.trim(),
      });
  ...
  const handleSendLocationPing = async (taskId: string) => {
    try {
      ...
      await api.post(`/logistics/${taskId}/location`, {
        latitude: lat,
        longitude: lng,
      });
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold bg-gradient-to-r from-cyan-400 to-teal-400 bg-clip-text text-transparent">
              MediShare Fleet & Volunteer Portal
            </h1>
